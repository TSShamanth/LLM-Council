...
export default function VerdictScreen({ verdictBreakdown, combinedOutput }) {
  ...
  const { winner, scores, overallAnalysis, minorityOpinions, revealMap } = verdictBreakdown;
  ...
  return (
    <div className="slide-up" style={{ display: "flex", flexDirection: "column", gap: "var(--sp-4)" }}>
      ...
      {/* ── User Choice: Winner vs Combined ────────────────────────── */}
      {combinedOutput && (
        <div style={{
          display: "flex", gap: "var(--sp-2)",
          background: "var(--bg-surface)", padding: "6px",
          borderRadius: "var(--r-lg)", border: "1px solid var(--border-dim)",
        }}>
          {[
            { id: "winner", label: "WINNER'S OUTPUT ONLY", icon: "🏆" },
            { id: "combined", label: "COMBINED BEST-OF-ALL", icon: "🔗" },
          ].map((mode) => (
            <button
              key={mode.id}
              ...
              style={{
                ...
                border: viewMode === mode.id ? "1px solid var(--accent-glow)" : "1px solid transparent",
                borderRadius: "var(--r-md)",
                color: viewMode === mode.id ? "var(--accent)" : "var(--text-muted)",
      ...
      {/* ── Combined Best-of-All Output ────────────────────────────── */}
      {viewMode === "combined" && combinedOutput && (
        <div style={{
          padding: "var(--sp-5)",
          ...
          <div className="mono" style={{ fontSize: 9, color: "var(--accent)", letterSpacing: 2, marginBottom: 12 }}>
            🔗 COMBINED BEST-OF-ALL OUTPUT — Generated by extracting the best features from every submission
          </div>
          <pre style={{
